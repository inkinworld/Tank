...
function Bullet(x,y,rota,direction,frame,type){
	...
	this.update = function(ctx,teamNum){
		var collTestList = [];
		var that =this;
							...
							if(collResult.position.dx < 0 ){
								...
								if( collResult.position.dy >= (Style.bullet/2) ){
									...
									if(item.list[2]){
										...
										// item.list[2] = 0;
										// item.list[3] = 0;
										var result = handleDestory(tileType,item.list,[2,3])
									...
									if(item.list[0]){
										...
										// item.list[0] = 0;
										// item.list[1] = 0;
										var result = handleDestory(tileType,item.list,[0,1])
										isBoom = result.isBoom;
										boomType = result.boomType;
							...
							if(collResult.position.dy < 0 ){
								...
								if( collResult.position.dx <= -(Style.bullet/2) ){
									if(item.list[0]){
										// item.list[0] = 0;
										// item.list[2] = 0;
										var result = handleDestory(tileType,item.list,[0,2])
										isBoom = result.isBoom;
										boomType = result.boomType;
									...
									if(item.list[1]){
										// item.list[1] = 0;
										// item.list[3] = 0;
										var result = handleDestory(tileType,item.list,[1,3])
										isBoom = result.isBoom;
										boomType = result.boomType;
							...
							if(collResult.position.dx < 0 ){
								...
								if( collResult.position.dy <= -(Style.bullet/2) ){
									if(item.list[1]){
										// item.list[1] = 0;
										// item.list[0] = 0;
										var result = handleDestory(tileType,item.list,[1,0])
										isBoom = result.isBoom;
										boomType = result.boomType;
		...
		function handleDestory(tileType,item,destoryList){
			switch(that.type){
				case 0:
					return bullet0Handle(tileType,item,destoryList);
				case 1:
					return bullet1Handle(tileType,item,destoryList);
			}
		}

		function bullet0Handle(tileType,item,destoryList){
			switch(tileType){
				case 0:
